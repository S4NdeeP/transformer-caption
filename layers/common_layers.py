...
def flatten4d3d_rem(x):
  """Flatten a 4d-tensor into a 3d-tensor by removing last dimension."""
  xshape = shape_list(x)
  result = tf.reshape(x, [xshape[0], xshape[1], xshape[2]])
  shape = result.get_shape().as_list()
  shape[2] = 2048
  result.set_shape(result.get_shape().merge_with(shape))
  # Assert shapes are fully known
  #result.get_shape().assert_is_fully_defined()
  return result
...
def initial_dense(inputs, filter_size, output_size):
  """Hidden layer with RELU activation followed by linear projection."""
  o = dense(inputs, output_size, use_bias=True, name="initial_conv")
  return o
...
def sample_with_temperature(logits, temperature):
  ...
  """
  #logits = tf.slice(logits, [0, 0], [-1, 9995])
  if temperature == 0.0:
