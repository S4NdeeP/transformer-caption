...
from tensor2tensor.models.vggnet import Vgg19
  ...
  def bottom(self, inputs):
    ...
    with tf.variable_scope(self.name):
      ...
      if not context.in_eager_mode():
        ...
        vggnet = Vgg19('data/t2t_train/imagenet-vgg-verydeep-19')
        #return tf.expand_dims(tf.slice(vggnet.build(inputs), [0, 0, 0], [-1, 10, 512]), 2)
        return tf.expand_dims(vggnet.build(inputs), 2)
      return inputs
