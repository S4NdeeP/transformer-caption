...
def encoder_decoder_attention_regularization(actual_attentions):
  """Computes encdec attention loss.
  ...
  # tensor with shape [batch_size, target_length, input_length].
  actual_attention_weights = tf.reduce_mean(actual_attention_weights, [0, 2])
  actual_attention_weights_all = tf.reduce_sum(actual_attention_weights, 1)
  #alpha_c = 1.0
  alpha_c = 0.0
  attention_reg = alpha_c * tf.reduce_sum((30. / 196 - actual_attention_weights_all) ** 2)
  tf.losses.add_loss(attention_reg)
...
@expert_utils.add_name_scope()
def add_timing_signal_nd_given_object_position(x, pos,  min_timescale=1.0, max_timescale=1.0e4):
  """Adds a bunch of sinusoids of different frequencies to a Tensor.
  ...
  """
  num_dims = 4
  channels = common_layers.shape_list(x)[-1] #// 4
  num_timescales = channels // (num_dims * 2)
  log_timescale_increment = (
      math.log(float(max_timescale) / float(min_timescale)) /
      (tf.to_float(num_timescales) - 1))
  ...
  dim = 0
  signal_sum = None
  for obj_dim in xrange(4):
    length = common_layers.shape_list(pos)[dim + 1]
    position = tf.to_float(tf.slice(pos,[0,0,obj_dim],[-1,-1,1]))
    scaled_time = position * tf.expand_dims(
        inv_timescales, 0)
    signal = tf.concat([tf.sin(scaled_time), tf.cos(scaled_time)], axis=2)
    prepad = obj_dim * 2 * num_timescales
    postpad = channels - (obj_dim + 1) * 2 * num_timescales
    signal = tf.pad(signal, [[0, 0], [0, 0], [prepad, postpad]])
    '''
    for _ in xrange(obj_dim):
      signal = tf.expand_dims(signal, 1)
    for _ in xrange(num_dims - 1 - obj_dim):
      signal = tf.expand_dims(signal, -2)
    '''
