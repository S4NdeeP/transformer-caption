...
class Transformer(t2t_model.T2TModel):
  ...
  def encode(self, inputs, target_space, hparams, features=None):
    ...
    """
    if not(use_bottom_up_features):
      inputs = common_layers.flatten4d3d(inputs)
    else :
      inputs = common_layers.flatten4d3d_rem(inputs)

      with tf.variable_scope("initial_ffn"):
        inputs = transformer_initial_ffn_layer(
          inputs, hparams, conv_padding="LEFT")
  ...
  def body(self, features):
    ...
    if self.has_input:
      if not(use_bottom_up_features):
        ...
        inputs = features["bottom_up_features"]
    ...
    if not(use_bottom_up_features):
        attention_loss = common_attention.encoder_decoder_attention_regularization(
  ...
  def _fast_decode(self,
    ...
    if hparams.pos == "timing" or hparams.pos == "none":
  ...
  if hparams.pos == "timing":
    if inputs_position is not None:
      ...
      if use_bottom_up_features:
        ...
        encoder_input = common_attention.add_timing_signal_nd_given_object_position(encoder_input, features['object_boxes'])

      else:
        encoder_input = tf.reshape(encoder_input, [-1,14,14,512])
        encoder_input = common_attention.add_timing_signal_nd(encoder_input)
        encoder_input = tf.reshape(encoder_input, [-1,196,512])
        #encoder_input = common_attention.add_timing_signal_1d(encoder_input)
...
def transformer_initial_ffn_layer(x,
                          hparams,
                          pad_remover=None,
                          conv_padding="LEFT"):
  ...
  conv_output = common_layers.initial_dense(
      x,
      hparams.filter_size,
      hparams.hidden_size)
  if pad_remover:
