...
class Vgg19(object):
    ...
    def build_inputs(self, input_image):
        ...
        with tf.variable_scope('encoder'):
            for i, layer in enumerate(layers):
                ...
                if layer_type == 'conv':
                    ...
                    self.params[layer_name]['w'] = tf.get_variable(layer_name+'/w', initializer=tf.constant(w), trainable=False)
                    self.params[layer_name]['b'] = tf.get_variable(layer_name+'/b',initializer=tf.constant(b), trainable=False)
