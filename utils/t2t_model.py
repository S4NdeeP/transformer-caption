...
from tensor2tensor.utils.bleu import evaluate,evaluate_captions,evaluate_captions_cider,evaluate_for_particular_captions
...
class T2TModel(base.Layer):
  ...
  def estimator_spec_eval(self, features, logits, labels, loss):
    ...
    '''
    label1 = tf.expand_dims(tf.expand_dims(features['infer_targets1'], -1), -1)
    label2 = tf.expand_dims(tf.expand_dims(features['infer_targets2'], -1), -1)
    label3 = tf.expand_dims(tf.expand_dims(features['infer_targets3'], -1), -1)
    label4 = tf.expand_dims(tf.expand_dims(features['infer_targets4'], -1), -1)

    labels = [labels, label1, label2, label3, label4]
  ...
  def estimator_spec_predict(self, features):
    ...
    predictions = {
        ...
        "targets_image": features.get("infer_image"),
