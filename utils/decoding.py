...
def log_decode_results(inputs,
  ...
  decoded_inputs = None
  if 'flickr_tokens32' in model_dir:
    data_dir = 'data/t2t_data_caption_flickr8k'
  else:
    data_dir = 'data/t2t_data_caption_bottom'

  if not(is_eval_server):
    val_caption_path = os.path.join(data_dir, "val_caption.txt")
    val_coco_id_path = os.path.join(data_dir, "val_image_id.txt")
    with open(val_caption_path, 'r') as f:
      val_labels = [line.rstrip('\n').split(':') for line in f]

    with open(val_coco_id_path, 'r') as f:
      val_coco_ids = [line.rstrip('\n') for line in f]

    val_label_dict = {}
    for image_id, label in val_labels:
      label = label.replace("'", '').replace('[', '').replace(']', '')
      image_label = [l.strip() for l in label.split(',')]
      val_label_dict[int(image_id)] = image_label

    val_coco_id_dict = {}

    for i, coco_image_id in enumerate(val_coco_ids, start=1):
      val_coco_id_dict[i] = coco_image_id
  ...
  if identity_output:
    ...
    if targets is not None:
      ...
      #decoded_targets = [targets_vocab.decode(_save_until_eos(target, is_image)) for target in targets]
      if not(is_eval_server):
        decoded_targets = val_label_dict[targets[0]]
        decoded_image_coco_id = val_coco_id_dict[targets[0]]
      else:
        decoded_image_coco_id = targets[0]
        decoded_targets = None

  tf.logging.info("Inference results OUTPUT: %s" % decoded_outputs)
  if targets is not None:
    tf.logging.info("Inference results TARGET: %s" % decoded_targets)
  return decoded_inputs, decoded_outputs, decoded_targets, decoded_image_coco_id
...
def decode_from_dataset(estimator,
  ...
  for problem_idx, problem_name in enumerate(problem_names):
    ...
    for num_predictions, prediction in enumerate(predictions):
      ...
      targets = prediction["targets_image"]
      outputs = prediction["outputs"]
      #print(prediction["targets_image"])
      #exit(0)
      # Log predictions
      decoded_outputs = []
      decoded_scores = []
      ...
      if decode_to_file:
        for i, (d_input, d_output, d_target, d_coco_id) in enumerate(decoded_outputs):
          candidate[my_counter] = [d_output]
          ref[my_counter] = d_target
          coco_id[my_counter] = d_coco_id
          my_counter += 1
          beam_score_str = ""
          if decode_hp.write_beam_scores:
    ...
    #print(candidate)
    save_pickle(ref, 'ref.pkl')
    save_pickle(candidate, 'candidate.pkl')
    save_pickle(coco_id, 'coco_id.pkl')
