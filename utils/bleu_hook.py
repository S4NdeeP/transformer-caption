...
from tensor2tensor.utils.bleu import evaluate,evaluate_captions,evaluate_captions_cider,evaluate_for_particular_captions, evaluate_captions_bleu_4

try:
  data_dir = 'data/t2t_data_caption_bottom'
  val_caption_path = os.path.join(
    data_dir,
    "val_caption.txt")

  with open(val_caption_path,'r') as f:
    val_labels = [
      line.rstrip(
        '\n').split(
        ':')
      for line in f]
  val_label_dict = {}
  for image_id, label in val_labels:
    label = label.replace(
      "'",
      '').replace(
      '[',
  ...
  val_captions_dict = val_label_dict
...
def decode_captions_for_blue_eval_metric(captions, idx_to_word):
  if captions.ndim == 1:
    T = captions.shape[0]
    N = 1
  else:
...
def bleu_4_metric_fn(image_ids, tokenized_output):

  caption_dict = val_captions_dict

  _, output = decode_captions_for_blue_eval_metric(tokenized_output, id_to_word)

  ref_all = []

  for id in np.nditer(image_ids):
    id = np.asscalar(id)
    caption_set = [caption + ' .' for caption in caption_dict[id]]
    ref_all.append(caption_set)

  score = [evaluate_captions_bleu_4([k], [v]) for k, v in zip(ref_all, output)]
  score = np.asarray(score).astype(np.float32)
  return score

def cider_metric_fn(image_ids, tokenized_output):
  ...
  score = evaluate_captions_cider(ref_all, output)
  score = np.asarray(score).astype(np.float32)
  return score
...
def bleu_4_score(predictions, labels, **unused_kwargs):
  """BLEU score computation between labels and predictions.
  ...
  '''
  labels[0] = tf.squeeze(labels[0], axis=[-1, -2])
  labels[1] = tf.squeeze(labels[1], axis=[-1, -2])
  labels[2] = tf.squeeze(labels[2], axis=[-1, -2])
  labels[3] = tf.squeeze(labels[3], axis=[-1, -2])
  labels[4] = tf.squeeze(labels[4], axis=[-1, -2])
  
  num_rows, num_cols = labels[0].get_shape().as_list()
  num_cols = tf.reshape(num_cols, [])
  paddings = tf.constant([[0, 0], [0, 25 - num_cols]])
  tf.pad(labels[0], paddings, "CONSTANT")
  print(labels)
  '''

  score = tf.py_func(bleu_4_metric_fn, [tf.squeeze(unused_kwargs['features']['infer_image']),
                                                  tf.squeeze(outputs)], tf.float32)
  return score, tf.constant(1.0)

def cider_score(predictions, labels, **unused_kwargs):
  outputs = tf.to_int32(tf.argmax(predictions, axis=-1))
  # Convert the outputs and labels to a [batch_size, input_length] tensor.
  outputs = tf.squeeze(outputs, axis=[-1, -2])
  labels = tf.squeeze(labels, axis=[-1, -2])

  score = tf.py_func(cider_metric_fn, [tf.squeeze(unused_kwargs['features']['infer_image']),
                                        tf.squeeze(outputs)], tf.float32)
