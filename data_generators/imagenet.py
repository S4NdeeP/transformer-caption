...
def imagenet_preprocess_example(example, mode, resize_size=None):
  ...
  resize_size = resize_size or [224, 224]
...
def preprocess_for_visualize(image, image_size=224):
  """Preprocesses the given image for evaluation.
  ...
  """
  image = _do_scale(image, image_size + 32)
  image_normalized = _normalize(image)
  image_normalized = _center_crop(image_normalized, image_size)
  image_normalized = tf.reshape(image_normalized, [image_size, image_size, 3])

  image_resized = _center_crop(image, image_size)
  image_resized = tf.reshape(image_resized, [image_size, image_size, 3])
